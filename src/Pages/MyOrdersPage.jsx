...
const MyOrdersPage = () => {
  ...
  useEffect(() => {
    ...
    const URL = `/my-orders?email=${user.email}`;
    axiosSecure.get(URL).then((res) => setMyOrders(res.data));
  }, [user, navigate, axiosSecure]);

  const handleDeleteBtn = (id) => {
    const proceed = confirm("Are you sure to delete your order?");
    if (proceed) {
      fetch(`https://car-doctor-server-murex-gamma.vercel.app/my-orders/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
